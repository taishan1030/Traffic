...
//    var stream1: DataStream[TrafficInfo] = streamEnv.addSource(
//      new FlinkKafkaConsumer[String]("topic_monitor_info", new SimpleStringSchema(), props).setStartFromTimestamp()
//    ).map(line=>{
//      var arr = line.split(",")
    ...
    stream1.keyBy(_.monitorId)
      ...
      .aggregate( //设计一个累加器：二元组(车速之后，车辆的数量)
        new AggregateFunction[TrafficInfo, (Double,Long),(Double,Long)] {
          ...
          override def add(value: TrafficInfo, acc: (Double, Long)): (Double, Long) = {(value.speed+acc._1, acc._2+1)}
